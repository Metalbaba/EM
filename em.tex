...
Key contributions include an oracle upper bound for clean comparison, a EM quality
...
\todo{Replace with precise numbers and findings once experiments are complete.}
...
optimised against the learned reward using PPO, subject to a
...
hard label. The loss for prompt $i$ is identical to the weighted DPO loss in 4.3.
...
The E-step is unchanged: $\gamma_i$ is still inferred from per-worker $\alpha_j$, $\beta_j$.
The M-step updates each worker's parameters as before, then updates the group hyperparameters
using moment matching:
\[
  a_g = \bar{\alpha}_g \cdot \kappa_g, \qquad
  b_g = (1-\bar{\alpha}_g) \cdot \kappa_g
\]
where $\bar{\alpha}_g = \frac{1}{|g|}\sum_{j\in g}\alpha_j$ is the group mean and
$\kappa_g > 0$ is a concentration parameter estimated from the empirical variance within
the group. The $\beta$ hyperparameters follow analogously.
...
\todo{Verify moment-matching M-step derivation against implementation, and add the
expertise-level variable if a separate $\kappa_g$ per group is used.}
...
\warn{Fix the train/test split and random seed \emph{before} running any experiment.
Any change to the split afterwards invalidates cross-phase comparisons.}

\todo{Specify exact row counts and the random seed(s) used.}
...
\todo{Fill in the empirical comparison once results are available: does joint EM converge
faster, reach higher accuracy, or degrade under strong initial policy bias?}
...
formulation and should be reviewed.
